...
public class Graph
{
    ...
    public List<int> GetRandomIndexesBeetweenIndexes(List<int> indexes, int countOfRandomIndexes = 1)
    {
        List<int> returnValue = new List<int>(indexes.Count * (indexes.Count - 1) / 2);
        int indexesAdded = 0, protectedCountOfRandomIndexes = 0;
        List<int> pathList;
        ...
        for (int i = 0; i < indexes.Count-1; i++)
        {
            ...
            {
                pathList = new List<int>(AStar(indexes[i], indexes[j]));
                indexesAdded = 0;
                ...
                if (pathList.Count < countOfRandomIndexes)
                    protectedCountOfRandomIndexes = pathList.Count;
                else 
                    protectedCountOfRandomIndexes = countOfRandomIndexes;
                //endless protection

                while (indexesAdded < protectedCountOfRandomIndexes)
                {
                    ...
                    if (returnValue.Contains(pathList[randomIndex]))
                        if (checkedIndexes.Count == pathList.Count)
                            break;
                        else
                            continue;

                    returnValue.Add(pathList[randomIndex]);
